...
class compute_node:
    def __init__(self):
        rospy.init_node('compute_node_1', anonymous=True)
        rospy.Subscriber("/cam_1", Float32MultiArray, self.callback_image1) 
        rospy.Subscriber("/vertical_permission", Int16, self.callback_intersc)#十字路口调度topic，1为南北向通行，即车可从b出库
        rospy.Subscriber("/enPermission_cam2", Int16, self.callback_exPermission)
        self.pub_move = rospy.Publisher('/move', Float32MultiArray, queue_size=1)
        self.pub_en_permission = rospy.Publisher('/enPermission_cam1', Int16, queue_size=1 )#用于判断IV区的车是否能进入I区
        self.image_flag = False
        self.flag_out = 0
        
        self.vertical_permission = 1#1-南北向通行 -1-东西向通行
        self.ex_permission = 1#1-可出1区 0-不可出1区
        ...
        #小车跑到离红绿灯近的区域时候才考虑启停
    def callback_exPermission(self, msg):#msg.data为1,允许南北向通行，为-1允许东西向通行
...
#主函数++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    def compute(self):
        ...
        #获得车号，并比get_id有一定鲁棒性
        def get_id_robust(id, point, theta = -100, radius=40):
            id_tmp = self.get_id(point,theta,radius=radius)
        ...
        def move_en_to_a():
            if self.id_en != 0:
                if if_filled(self.pose_now, Point_a) == 1 and self.get_id(Point_a) == self.id_en:
                    rospy.loginfo("id_en:%s,waiting at a",self.id_en)
                    self.send_stop(self.id_en)
                    # self.id_a = self.id_en
                    self.id_en = 0
        ...
        def move_b_to_ex():#外围函数需要判断下个场景入口是否有车
            if self.id_ex != 0:#b-ex段有车
                # if TRUE:
               if self.vertical_permission == 1 and self.ex_permission == 1:#允许南北向车通行 且2区允许进入
            ...
            #     print("no car at b")
                ...
                # #a-p
        #output:正在执行a-p操作return true, 没有执行a-p return false
        def move_a_p_new(id):#id需要外围语句判断，此时id_en会变成id_ap
            ...
            if self.id_p1 * self.id_p2 == 0:#可以执行a-p操作，函数return 1    
                    ...
                    if self.is_reverse == 0:#前进
                        ...
                        if id in self.ids:
                            if if_filled_specific(self.pose_now[self.ids.index(id)], way_1.way_ap1[1])== 1:#判断是否到达倒车起始点
                                self.is_reverse = 1
                                self.send_stop(id)
                                rospy.loginfo("id_ap:%s, a-p1, stop to reverse", self.id_ap)
                            else:#直行且未进入倒车其实点
                                rospy.loginfo("id_ap:%s, a-p1, forward", self.id_ap)
                        ...
                        if id in self.ids:#有时候识别不稳定，避免报错的下策                       
                            if if_filled_specific(self.pose_now[self.ids.index(id)], way_1.Point_p1)==1:#到达车库 待改进：到达车库时，id_p1!=0 因此程序跳不到这步！！！！
                                rospy.loginfo("id_ap:%s, a-p1, arrive at p1", self.id_ap)
                                self.send_stop(id)
                                ...
                                rospy.loginfo("id_ap:%s, a-p1, reversing", self.id_ap)
                    ...
                    if self.is_reverse == 0:#前进
                        ...
                        if id in self.ids:
                            if if_filled_specific(self.pose_now[self.ids.index(id)], way_1.way_ap2[1])== 1:#判断是否到达倒车起始点
                                self.is_reverse = 1
                                self.send_stop(id)
                                rospy.loginfo("id_ap:%s, a-p2, stop to reverse", self.id_ap)
                            #else:
                            elif if_filled_specific(self.pose_now[self.ids.index(id)], way_1.way_ap2[1])== 0:#直行且未进入倒车其实点
                                rospy.loginfo("id_ap:%s, a-p2, forward", self.id_ap)
                        ...
                        if id in self.ids:#有时候识别不稳定，避免报错的下策                       
                            if if_filled_specific(self.pose_now[self.ids.index(id)], way_1.Point_p2)==1:#到达车库
                                rospy.loginfo("id_ap:%s, a-p2, arrive at p2", self.id_ap)
                                self.send_stop(id)
                                self.is_reverse = 0#reset
                                self.id_ap = 0#reset
                                self.id_a = 0
                ...
                if self.id_a != 0:    
                    self.send_stop(self.id_a)
                    self.id_a = 0
                    rospy.loginfo("parking lot is full")
                    return False
            ...
            if id in self.ids:#摄像头识别不稳定，没有识别到小车上的二维码（不加这句可能报错）
                if id == self.id_p1:#p1-b
                    ...
                    if if_filled(self.pose_now, way_1.Point_b)==1:
                        ...
                        rospy.loginfo("id_pb:%s, p1 arrive at b", id)
                        self.send_stop(id)
                        return True
                    else:#没有到达b时
                        # print("id_pb:", id, ", moving to b")
                        rospy.loginfo("id_pb:%s, p1 moving to b", id)
                    ...
                    if if_filled(self.pose_now, way_1.Point_b)==1:
                        rospy.loginfo("id_pb:%s, p2 arrive at b", id)
                        self.send_stop(id)
                        return True
                    else:#没有到达b时
                        rospy.loginfo("id_pb:%s, p2 moving to b", id)
        ...
        while not rospy.is_shutdown():
            if self.image_flag == True:
                rospy.loginfo("id_en:%s, id_a:%s, id_ap:%s, id_p1: %s, id_p2:%s, id_pb:%s, id_intersc:%s, id_ex:%s", self.id_en, self.id_a, self.id_ap, self.id_p1, self.id_p2, self.id_pb, self.id_intersec_ctrl, self.id_ex)
                if id in self.ids:
                    rospy.loginfo(self.get_id(Point_a),if_filled_specific(self.pose_now[self.ids.index(id)], way_1.way_ap1[1]))

                #-------------------------------------------
                #获取各个检测点的车号和状态,一旦识别到车号，小车离开该点需要程序手动置为零
                self.id_ex = get_id_robust(self.id_ex,Point_b)
                self.id_en = get_id_robust(self.id_en,Point_en,3)
                self.id_p1 = get_id_robust(self.id_p1,way_1.Point_p1,radius = 30)
                self.id_p2 = get_id_robust(self.id_p2,way_1.Point_p2,radius = 30)

                self.id_b = get_id_robust(self.id_b, Point_b)
                self.id_a = get_id_robust(self.id_a, Point_a)
                self.id_intersec_ctrl = get_id_robust(self.id_intersec_ctrl, way_1.Point_intersc_check)
                # print(self.id_en,self.id_ex,self.id_p1,self.id_p2,self.id_ap,self.id_pb)
                ...
                # a-p  
                if self.is_lot_occupied == 0:
                # if self.is_lot_occupied == 0:
                    self.id_ap = get_id_robust(self.id_ap, Point_a)
                    # if self.id_a == self.id_ap:#补丁，以免id_a id_ap对应同一个车
                    #     self.id_a  = 0
                    if self.id_ap != 0:
                        is_ap = move_a_p_new(self.id_ap)
                        # if is_ap == 0:#没有执行ap操作时
                        if is_ap == 0 and self.id_pb == 0:
                            self.id_ap = self.id_a 
                        elif is_ap == 1:
                            self.is_entrance_occupied = 0
                        # else:#move_ap返回None的情况，为何为None未知
                        #     self.id_ap = self.id_a
                elif (self.id_ap != 0 and self.id_pb == 0):
                        is_ap = move_a_p_new(self.id_ap)
                        if is_ap == 0 and self.id_ap == 0:#没有执行ap操作时
                            self.id_ap = 0
                        elif is_ap == 1:
                            self.is_entrance_occupied = 0
            ...
            is_permit = 1
            if self.id_en != 0 or self.id_ex != 0 or self.id_ap != 0 or self.id_a != 0:
                is_permit = 0
            else:
                is_permit = 1
            self.pub_en_permission.publish(Int16(data = is_permit))#待修改：将用十字路口调度节点替代这一功能
