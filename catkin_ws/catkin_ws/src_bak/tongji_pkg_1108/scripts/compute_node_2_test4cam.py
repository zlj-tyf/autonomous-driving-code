...
#路径
path_p1a, path_ab,path_b8= way_2.way()
#轴距
l = l = 45.641
...
class compute_node:
    def __init__(self):
        rospy.init_node('compute_node_2', anonymous=True)
        rospy.Subscriber("/cam_2", Float32MultiArray, self.callback_image2) 
        # rospy.Subscriber("/cam_3", Float32MultiArray, self.callback_image3) 
        rospy.Subscriber("/vertical_permission", Int16, self.callback_intersc)#十字路口调度topic，-1为东西向通行，即车可从b出库
        rospy.Subscriber("/enPermission_cam3",Int16,self.callback_enpermission)
        self.pub_move = rospy.Publisher('/move', Float32MultiArray, queue_size=1)
        self.pub_en_permission = rospy.Publisher("/enPermission_cam2",Int16, queue_size = 1)
        ...
        self.en_permission = 1 #1-允许进入2区 0-不允许进入2区
    ...
    def callback_image3(self, msg):   
                #记录到第五帧时，统计入口没车的帧数，若大于1则认为入口有车，即flag_out = 1
        if self.out_count_all == 6:
            if self.out_count_now > 1:
                self.flag_out = 0
            else:
                self.flag_out = 1
            #重置计数器
            ...
            if len(msg.data) != 0 :
                for i in range(len(msg.data)//4):      #判断本帧3区每一个是否靠近小车入口a，若是，则out_count_now +=1
                    if distance(msg.data[i*4+1], msg.data[i*4+2], way_3.Point_a[0], way_3.Point_a[1]) < 40 and msg.data[i*4] < 10:#III区a点有车
    ...
    def compute(self):
        ...
        l = 40  #车轴距
        Lf = 80 #前视距离
        Point_a = way_2.Point_a
        Point_b = way_2.Point_b
        Point_8 = way_2.Point_8
        Point_1 = way_2.Point_1
        ...
        while not rospy.is_shutdown():
            if self.image_flag == True:
                    rospy.loginfo("id_1:%s, id_ab:%s, id_b8: %s", self.id_1, self.id_ab, self.id_b8)
                    #-------------------------------------------
                    #获取各个检测点的车号
                    id_1_tmp = self.get_id(Point_1,-1.58)
                    id_ab_tmp = self.get_id(Point_a,-1.58)
                    id_b8_tmp = self.get_id(Point_b,3.14)
                    id_a_tmp = self.get_id(Point_a,-1.58)#a
                    id_b_tmp = self.get_id(Point_b, 3.14)
                    #检测点1 a b 8,停车点a b
                    
                    #获取各个检测点的状态
                    if id_1_tmp != 0 and id_1_tmp in self.ids:#防止毛刺使得id_x误识别 Q:为何对于id_1的赋值没有self.id_1 == 0的判断？A:因为en_permission话题可以保证en-a段有车时候没有车进入2区
                        flag_1a = 1#1-a有车
                        self.id_1 = id_1_tmp
                    ...
                    # if id_b8_tmp != 0 and id_b8_tmp in self.ids and self.id_b8 == 0:
                    #     flag_b = 1
                    ...
                    if self.id_1 != 0:
                        if if_filled(self.pose_now, Point_a) == 0:
                            ...
                            rospy.loginfo("id_1:%s,en-a",self.id_1)
                        else:
                            rospy.loginfo("id_1:%s,arrive at a", self.id_1)
                            self.send_stop(self.id_1)
                            #self.id_ab = self.id_1
                            #防止id_1经过a点被置0，而ab段有别的车，导致id_1失控
                            if self.id_ab == 0:
                                self.id_1 = 0
                            else:
                                self.send_stop(self.id_1)
                    ...
                    # #a-b    
                    # if self.id_ab != 0:
                    #     # print("id_ab:", self.id_ab)
                    #     if self.id_b8 != 0:
                    #         self.send_stop(self.id_ab)
                    #         rospy.loginfo("stop at a, id_ab: %s", self.id_ab)
                    #         # print("id_b8:",self.id_b8)
                    #         # print("-----------")
                    #     else:
                    #         #a-b巡线
                    ...
                    #             rospy.loginfo("id_ab:%s,arrive at b",self.id_ab)
