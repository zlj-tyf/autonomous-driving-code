...
class compute_node:
    def __init__(self):
        ...
        rospy.Subscriber("/enPermission_cam4", Int16, self.callback_exPermission)
        self.pub_move = rospy.Publisher('/move', Float32MultiArray, queue_size=1)
        self.pub_en_permission = rospy.Publisher('/enPermission_cam2', Int16, queue_size=1 )#用于判断IV区的车是否能进入I区
    ...
    def compute(self):
        ...
        while not rospy.is_shutdown():
            if self.image_flag == True:
                ...
                rospy.loginfo("id_en:%s, id_ex:%s, id_p1:%s, id_p2:%s, id_ap:%s, id_pb:%s,v_Per:%s,ex_Per:%s", self.id_en,self.id_ex,self.id_p1,self.id_p2,self.id_ap,self.id_pb, self.vertical_permission,self.ex_permission)
